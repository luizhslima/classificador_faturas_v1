...
\begin{itemize}
	...
	\item \codepath{notebooks/experimentos/} --- \textbf{esteira experimental reprodutível} (\texttt{common.py} e as etapas \texttt{01} a \texttt{10}) que gera integralmente os resultados do Capítulo~\ref{Cap:Ava_Exp}: avaliação de OCR, treino e teste cego das linhas de base clássicas, ajuste do ByT5, busca vetorial densa, execução do agente híbrido, estudo de ablação, consolidação das tabelas e figuras em \LaTeX, intervalos de confiança e testes de significância, e verificação empírica de conformidade com a LGPD;
