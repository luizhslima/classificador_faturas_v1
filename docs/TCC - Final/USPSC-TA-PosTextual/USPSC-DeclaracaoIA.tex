...
\vspace{0.12cm}
\noindent\checkedbox\hspace{0.12cm}\textbf{Medidas adotadas:} minimização de dados sensíveis (números completos de cartão, CVV e senhas expurgados na ingestão), ausência de CPF ou documento do titular na base (verificada empiricamente por consulta à base de produção) e processamento integralmente local, sem envio a APIs de terceiros (Seção~\ref{sec:sol_lgpd}); o acervo de faturas reais e os respectivos arquivos de verdade de referência foram deliberadamente excluídos da distribuição pública do repositório (Apêndice~\ref{apendice:repositorio}).
