...
\begin{figure}[htb]
	...
	\resizebox{0.95\textwidth}{!}{%
		\begin{tikzpicture}[
			...
			\node[box, fill=gray!10]   (user)  at (-5.6, 0)  {Usuário / Cliente \\ (Upload da Fatura)};
			...
			\node[box, fill=green!10]     (ocr)        at (0, -3.6)     {Worker Extração \\ (Docling / Tesseract)};
			...
			\node[storage, fill=teal!10] (silver)     at (5.6, -7.4) {Postgres + PGVector \\ (Dados e Embeddings)};
			...
			\draw[dashed_arrow] (byt5.east) -- node[elabel] {Loga Métricas} (mlops.west);
...
\begin{itemize}
	...
	\item \textbf{Apache Kafka:} Barramento de mensageria distribuído que garante o desacoplamento temporal e a resiliência volumétrica entre a camada de ingestão e os workers assíncronos através dos tópicos \texttt{test-topic}, \texttt{events-topic} e \codepath{datalake-bronze-ingestao-topic};
...
\begin{enumerate}
	...
	\item \textbf{Nó \texttt{buscar\_por\_similaridade}:} Executa a busca por similaridade de cosseno na extensão PGVector utilizando os embeddings gerados pelo modelo \modelemb{}. Se a melhor correspondência apresentar escore de similaridade $\ge 0.85$, a transação é considerada classificada com alta certeza, sendo roteada diretamente para o nó \texttt{salvar\_resultado};
...
\begin{itemize}
	\item \textbf{Testes Unitários:} Verificação isolada das funções determinísticas de pré-processamento --- saneamento léxico com \textit{Regex}, \textit{parsers} de datas e valores monetários, resolução de \textit{aliases} comerciais e contratos Pydantic --- por meio de \textit{pytest} (com suporte a execução assíncrona) no núcleo Python. O serviço de ingestão reativa em Java conta com verificação de carga do contexto da aplicação \textit{Spring Boot} via \textit{JUnit~5};
	\item \textbf{Testes de Regressão de Modelo:} O pipeline experimental reprodutível (\codepath{notebooks/experimentos/}, etapas 01 a 08) fixa a semente aleatória e um conjunto de teste-cego particionado por estabelecimento (sem vazamento de \textit{merchant}), permitindo detectar regressões nas métricas preditivas a cada alteração de código, \textit{prompt} ou hiperparâmetro, com os resultados versionados no MLflow;
	\item \textbf{Validação de Integração Assistida:} A comunicação de ponta a ponta entre a API de ingestão, o barramento Kafka, o \textit{Object Storage} MinIO e o \textit{worker} de extração foi exercitada de forma assistida em ambiente \textit{Docker Compose}, confirmando a publicação e o consumo de eventos e a persistência dos artefatos nas camadas do Data Lakehouse.
\end{itemize}
