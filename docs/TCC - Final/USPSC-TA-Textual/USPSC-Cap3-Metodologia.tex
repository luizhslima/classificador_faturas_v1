...
\end{itemize}
Em termos de teste estatístico, cada subhipótese é a hipótese alternativa de um par formalizado a priori: para H1a, $H_0$: acurácia populacional $\le 85\%$ contra $H_1$: acurácia populacional $> 85\%$, verificado por teste binomial exato unilateral e pelo intervalo de confiança de Wilson sobre a proporção de acertos; para H1b, $H_0$: F1-Macro populacional $\le 85\%$ contra $H_1$: F1-Macro populacional $> 85\%$, verificado pelo intervalo de confiança de 95\% obtido por reamostragem \textit{bootstrap}. Adota-se o nível de significância $\alpha = 0{,}05$; uma subhipótese é considerada sustentada somente se a estimativa pontual supera o limiar \textit{e} o teste rejeita $H_0$, reportando-se o intervalo de confiança como medida complementar da incerteza. Os procedimentos são detalhados na Seção~\ref{sec:val_significancia}. Como a formulação original é conjuntiva (\textit{E lógico} entre H1a e H1b), a hipótese central como um todo é considerada refutada caso qualquer uma das subhipóteses seja refutada, independentemente do resultado da outra. O \textbf{objeto} da hipótese é a esteira híbrida enunciada -- isto é, o agente em grafo de estados descrito na Seção~\ref{sec:sol_agente_inteligente} --, e é sobre ela que as duas subhipóteses são verificadas (Seção~\ref{sec:val_respostas_questoes}). As demais arquiteturas avaliadas (linhas de base esparsas, modelo no nível de byte e busca vetorial isolada) cumprem o papel de referência comparativa exigido pela Q1; seus resultados são reportados frente ao mesmo limiar de 85\%, mas não substituem a esteira híbrida como objeto da verificação.
...
\begin{enumerate}
	...
	\item \textbf{Compreensão dos Dados:} Coleta e exploração de conjuntos de dados públicos oficiais (CPGF) e de faturas reais pseudonimizadas (Seção~\ref{sec:sol_lgpd}), com inspeção qualitativa do ruído nativo introduzido por intermediadores de pagamento e terminais de captura;
	\item \textbf{Preparação dos Dados:} Engenharia de extração com preservação de leiaute (Docling V2) e comparação experimental com reconhecimento óptico (Tesseract OCR), normalização léxica com Regex, descarte de jargões corporativos e cálculo de \textit{embeddings} vetoriais;
...
\begin{figure}[htb]
	...
	\resizebox{0.95\textwidth}{!}{%
		\begin{tikzpicture}[
			...
			\node[box, fill=green!10]     (ocr)        at (0, -3.6)     {Worker Extração \\ (Docling V2)};
...
A ferramenta de busca autônoma na web (\texttt{duckduckgo\_search}), descrita nos itens acima como parte da engenharia do agente, é habilitada por variável de ambiente (\texttt{AGENTE\_WEB}) e foi exercitada em uma execução complementar do teste cego, na qual os \textit{backends} de busca do pacote cliente falharam por erros de conexão na maior parte das chamadas (registro na Seção~\ref{sec:val_ambiente}). Os resultados principais do Capítulo~\ref{Cap:Ava_Exp} correspondem à execução com a ferramenta desabilitada; a execução com a ferramenta é reportada como evidência complementar.
...
\begin{itemize}
	...
	\item \textbf{Testes Unitários:} O ambiente Python está preparado para \textit{pytest} (com suporte a execução assíncrona), mas a suíte de testes unitários das funções determinísticas de pré-processamento --- saneamento léxico com \textit{Regex}, analisadores (\textit{parsers}) de datas e valores monetários, resolução de \textit{aliases} comerciais e contratos Pydantic --- \textbf{não foi escrita} nesta versão; o serviço de ingestão em Java conta apenas com a verificação de carga do contexto \textit{Spring Boot} via \textit{JUnit~5}. A ausência dessa suíte explica, em parte, que defeitos determinísticos do agente (como a chave de \textit{marketplace} não casada, Apêndice~\ref{apendice:normalizacao}) tenham sido identificados apenas na revisão final do código.
\end{itemize}
